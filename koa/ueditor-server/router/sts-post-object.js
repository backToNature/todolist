    ...
    return new Promise((resolve, reject) => {
        ...
        var policy = {
            ...
            'statement': [{
                ...
                'principal': {'qcs': ['*']},
                'resource': [
                    'qcs::cos:' + config.Region + 'ap-shanghai:uid/' + AppId + ':prefix//' + AppId + '/' + ShortBucketName + keyPath
                ]
            }]
        };

        var policyStr = JSON.stringify(policy);
        // policyStr = policyStr.replace(/\"/g, '%22');
        ...
        // request.post(config.Url, {form: params}, (err,response,body) => {
        //     try {
...
//     tempKeystempKeys = await getTempKeys(key);

//     console.log(tempKeystempKeys);
// })();
...
module.exports = async (ctx, next) => {
    ...
    let tempKeys;
    try {
        tempKeys = await getTempKeys(key);
    } catch (e) {
        ctx.body = {
            err: e
        };
